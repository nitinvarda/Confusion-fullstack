...
router.route('/')
    ...
    }).post(protect, admin, upload.single("dishImage"), (req, res, next) => {
...
router.route('/:dishId/comments')
    ...
    .get((req, res, next) => {
        ...
        Dishes.findById(req.params.dishId)
            .then(dish => {
                ...
                if (dish != null) {
                    ...
                    const exists = dish.comments.find(comment => comment.author.toString() === req.user._id.toString())
                    if (exists) {
                        err = new Error('You Already Commented on this dish')
                        err.status = 404;
                        return next(err)
        ...
        Dishes.findById(req.params.dishId)
            ...
            .then(dish => {
                if (dish != null && dish.comments.length >= 1) {
                    ...
                    err = new Error('Comments for  ' + req.params.dishId + ' already empty')
                    err.status = 404;
                    return next(err)
...
// /:dishId/comments/:commentId dishId router
            ...
            .then(dish => {
                ...
                if (dish != null && dish.comments.id(req.params.commentId) != null) {
                    ...
                    if (dish.comments.id(req.params.commentId).author._id.equals(req.user._id)) {
                        ...
                        err = new Error('You cannot modify or delete other comments which are not posted by you')
                        err.status = 404;
                        return next(err)
